// Same as in `bevy::winit`, but modified to allow inserting vulkano renderer that holds the window
use bevy::ecs::{
    entity::Entity,
    event::EventWriter,
...
use bevy::utils::tracing::{error, info, warn};
use bevy::window::{
...
use crate::{
    ...
    get_best_videomode, get_fitting_videomode, CreateWindowParams, BevyVulkanoWindows,
};
...
pub fn create_windows<F: QueryFilter + 'static>(
    ...
    for (entity, mut window, handle_holder) in &mut created_windows {
        ...
        if let Some(theme) = vulkano_window.window().theme() {
            window.window_theme = Some(convert_winit_theme(theme));
        }

        window
            .resolution
            .set_scale_factor_and_apply_to_physical_size(vulkano_window.window().scale_factor() as f32);
        ...
        // if let Ok(handle_wrapper) = RawHandleWrapper::new(&WindowWrapper::new(vulkano_window.window())) {
...
pub(crate) fn despawn_windows(
    ...
    for window in closed.read() {
        ...
        if !window_entities.contains(window) {
            if let Some(window) = vulkano_windows.remove_window(window) {
            }
...
/// # Notes
///
/// - [`Window::present_mode`] and [`Window::composite_alpha_mode`] changes are handled by the `bevy::render` crate.
...
pub(crate) fn changed_windows(
    ...
    for (entity, mut window, mut cache) in &mut changed_windows {
        let Some(winit_window) = vulkano_windows.get_window(entity) else {
            continue;
        };

        let winit_window = winit_window.window();
        ...
        if window.cursor.grab_mode != cache.window.cursor.grab_mode {
            crate::vulkano_windows::attempt_grab(winit_window, window.cursor.grab_mode);
        }
