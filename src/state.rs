// Pretty much the same as is found in bevy::winit! Just swapped BevyVulkanoWindows to BevyVulkanoWindows, and dealt with the consequences. Also removed that annoying event.
use approx::relative_eq;
use bevy::app::{App, AppExit, PluginsState};
use bevy::ecs::change_detection::{DetectChanges, NonSendMut, Res};
...
use crate::{
    ...
    WinitSettings, BevyVulkanoWindows,
};
...
struct WinitAppRunnerState {
    ...
    event_writer_system_state: SystemState<(
        ...
        NonSend<'static, BevyVulkanoWindows>,
...
impl ApplicationHandler for WinitAppRunnerState {
    fn new_events(&mut self, event_loop: &ActiveEventLoop, cause: StartCause) {
        ...
        #[cfg(feature = "trace")]
        let _span = bevy::utils::tracing::info_span!("winit event_handler").entered();
        ...
        if self.lifecycle == AppLifecycle::WillResume {
            ...
            #[cfg(target_os = "android")]
            {
                ...
                let mut query = self.world_mut()
                    .query_filtered::<(Entity, &Window), (With<CachedWindow>, Without<bevy::window::RawHandleWrapper>)>();
        ...
        match update_mode {
            UpdateMode::Continuous => {
                ...
                cfg_if::cfg_if! {
                    ...
                    {
                        let winit_windows = self.world().non_send_resource::<BevyVulkanoWindows>();
                        let visible = winit_windows.windows.iter().any(|(_, w)| {
                            w.window().is_visible().unwrap_or(false)
                        });
